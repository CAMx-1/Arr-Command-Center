...
function buildBottomNav() {
  ...
  const svcHex = (svc) => {
    const meta = SERVICE_META[svc.type] || {};
    const st = state.status[svc.key];
    const btn = h('button', { class: `bn-hex ${route === svc.key ? 'active' : ''}`, title: `${svc.label} · press and hold for shortcuts`, 'aria-label': svc.label, 'aria-description': 'Press and hold for service shortcuts', dataset: { svcKey: svc.key }, onclick: () => { haptic(); location.hash = `#/${svc.key}`; } },
  ...
  const home = h('button', { class: `bn-hex bn-home ${route === 'home' ? 'active' : ''}`, title: 'Home', 'aria-label': 'Home', onclick: () => { haptic(); location.hash = '#/home'; } },
...
function renderAllServicesGrid() {
  ...
  mount(sheet,
    ...
    h('div', { class: 'allsvc-hint' }, 'Tap to open · press and hold bottom icons for shortcuts · manage favorites in Settings'),
...
function toggleNotif(open) {
  ...
  if (notifOpen) {
    positionNotifPanel(); renderNotifPanel(); markSeen(); updateNotifBadge();
...
async function refreshStatus() {
  try {
    ...
    recordClientEvent('warn', 'Status refresh failed', err?.message || err);
    setConnBanner(navigator.onLine ? 'server' : 'offline');
    startProbing();
  }
}

// ----- Connection banner -----
function setConnBanner(mode, detail = '') {
  const el = document.getElementById('conn-banner');
  if (!el) return;
  if (!mode) { el.classList.remove('show'); el.removeAttribute('data-mode'); el.textContent = ''; return; }
  const labels = {
    offline: 'Offline — check your network connection',
    reconnecting: 'Reconnecting to Arr Command Center…',
    server: 'Server unavailable',
    auth: 'Your sign-in has expired',
  };
  el.dataset.mode = mode;
  const retry = h('button', {
    class: 'conn-retry', type: 'button',
    onclick: () => { recordClientEvent('info', 'Manual connection retry'); stopProbing(); startProbing(); },
  }, mode === 'auth' ? 'Sign in' : 'Retry');
  mount(el,
    h('span', { class: 'conn-message' }, detail || labels[mode] || labels.reconnecting),
    retry,
  );
...
async function init() {
  ...
  recordClientEvent('info', 'Initial route rendered', currentRoute());
  ...
  // Version-aware refresh: config identifies the deployment that supplied this
  // page, allowing one automatic cache-busted reload when it changes.
  initUpdateBanner({ loadedBuild: state.config.buildId });

  // A warm native resume keeps user context but refreshes status and notification
  // counts immediately instead of waiting for their polling intervals.
  onNativeAppStateChange(({ isActive }) => {
    if (!isActive) return;
    recordClientEvent('info', 'App returned to foreground');
    refreshStatus();
    refreshNotifications();
  });
